...
4. ownedCoupon.use(userId, orderId)             ← validateUsable + USED 전이 + orderId 기록 (R13~R15)
...
public OrderResult.OrderSummary createOrder(Long userId, OrderCriteria.Create criteria) {
    ...
    return OrderResult.OrderSummary.from(
            orderService.createOrder(userId, items, discountAmount));
}
```

> 주의: `useAndCalculateDiscount`에 orderId를 전달하려면 Order를 먼저 저장해야 한다. 실제 구현 시 순서 조정이 필요하다. (Order 먼저 생성 → couponService.use에 orderId 전달 → Order에 discountAmount 반영)
