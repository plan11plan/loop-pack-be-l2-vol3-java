---
name: deep-dive
description: "Use when practicing technical interview preparation through DFS-style deep questioning. Triggers: '깊이 파기', 'deep dive', '면접 학습', '꼬리질문 학습', 'DFS 학습', '장애 시나리오 학습'. 장애 시나리오 기반 꼬리질문, 면접 답변 스크립트, 블로그 글 생성."
---
...
주제별 DFS 질문 트리 템플릿: [dfs-templates.md](references/dfs-templates.md)

Spring Transaction, Nginx 무중단 배포, 동시성/데이터 정합성 3개 주제의 DFS 트리 예시를 포함한다. 시나리오 설계 시 참고한다.
...
세션 진행 시 자주 발생하는 실수와 해결 방법이다.

| 실수 | 해결 |
|------|------|
| 사용자가 답변하기 전에 답을 먼저 말해버림 | 질문만 던지고 멈춘다. 침묵이 길어져도 힌트는 사용자가 시도한 후에만 제공 |
| 한 번에 여러 질문을 동시에 던짐 | 반드시 1개씩만 질문한다. 다음 질문은 현재 답변을 받은 후에 결정 |
| 핵심 키워드가 나왔는데 스치고 지나감 | 꼬리질문 트리거 키워드 테이블을 확인하고, 해당 키워드가 나오면 반드시 더 깊이 파고든다 |
| Depth를 건너뛰고 갑자기 코드 레벨로 점프 | DFS 트리 순서를 따른다. 프레임워크 레벨(D2-3)을 거치지 않고 코드 레벨(D4-5)로 넘어가지 않는다 |
| 블로그 글에서 사용자의 틀린 답변을 그대로 노출 | "흔한 오해" 리프레이밍으로 변환한다. 개인 기록이 아닌 독자에게 가치 있는 콘텐츠로 작성 |
